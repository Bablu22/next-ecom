...
interface Props {
  ...
  onSubmit(values: INewProduct): void;
...
export default function ProductForm(props: Props) {
  ...
  const removeImage = async (index: number) => {
    ...
    const clouSourceUrl = "https://res.cloudinary.com";
    if (imageToremove.startsWith(clouSourceUrl)) {
      ...
      const newfile = images.filter((_, i) => {
        if (i !== indexToRemove) return;
      });
      setImages([...newfile]);
    }
    const newfile = productImagesSource.filter((_, i) => {
      if (i !== index) return;
    });
    setProductImagesSource([...newfile]);
  };
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto bg-white  border">
      <h1 className="text-xl border-b mb-3">Add new product</h1>
      ...
      >
        ...
        <Button placeholder="" disabled={isPending} type="submit">
          {getBtnTitle()}
        </Button>
      </form>
    </div>
